...
COST_MICRO = 1.1     # pts round turn, MNQ ($2/pt)  -- NQ ONLY
COST_EMINI = 0.725   # pts round turn, NQ ($20/pt)   -- NQ ONLY

# Cost in POINTS is instrument-specific and must never be carried across.
# ES has ~1/7 the point volatility of NQ, so the NQ figure would be a ~3x
# overstatement. Use cost_for() or pass cost= explicitly.
COSTS = {
    "NQ": {"micro": 1.10,   "emini": 0.725},   # MNQ $2.20 / NQ  $14.50 round turn
    "ES": {"micro": 0.59,   "emini": 0.44},    # MES $2.95 / ES  $22.00 round turn
    "CL": {"micro": 0.027,  "emini": 0.0195},  # MCL $2.70 / CL  $19.50 round turn
    "GC": {"micro": 0.27,   "emini": 0.195},   # MGC $2.70 / GC  $19.50 round turn
    # SIL has only ONE size (Micro Silver, 1,000 oz, $1,000/pt, tick 0.005 = $5.00).
    # SI (5,000 oz) and QI (2,500 oz) are the larger contracts, not in this dataset.
    # 1 tick + $1.70 commission = $6.70 = 0.0067 pts. But SIL's book is thin --
    # 12.7% of 5m bars trade under 50 contracts -- so a 1-tick spread is optimistic.
    # 0.0117 (2-tick) is the realistic figure; run both and report the worse one.
    "SIL": {"micro": 0.0117, "emini": 0.0117},
}
# Cost as a share of median 5m ATR14 — the number that actually governs whether a
# tight-stop reversal system can work at all:
#   MNQ 4.1%   NQ 2.7%   |   MGC 5.7%   GC 4.1%   |   SIL 8.6% (2-tick spread)
#   MES 15.4%  ES 11.5%  |   MCL 14.2%  CL 10.2%
# ATR14 in TICKS on 5m: NQ 108, GC 47, SIL 27, CL 19, ES 15. Granularity matters
# as much as cost: a 0.8 ATR stop is 86 ticks on NQ but only 12 on ES.
# RISK PER TRADE at the 0.8 ATR stop, smallest tradeable size:
#   MES $15  MCL $15  MGC $38  MNQ $43  SIL $108  <- SIL is 2.5x the MNQ unit and
# has no smaller version, so it does not fit a $2,000 trailing-drawdown account.
# NQ is the outlier in the strategy's favour. A weaker result on ES or CL is
# partly predicted by this alone and is NOT by itself evidence of NQ-specificity.

def cost_for(instrument="NQ", size="micro"):
    return COSTS[instrument.upper()][size]

# 15 Jun 2026: STILL UNRESOLVED (see notes). NQ +1.73%, ES +1.44%, GC +1.17%,
# CL -3.43% at the same session boundary. Could be a market-wide weekend event
# or simultaneous roll splices; the sign pattern fits BOTH. Decisive test needs
# single-contract series (ESU2026 vs ESM2026), not the continuous NQ1!/ES1!.
# Consequence: it is real risk the strategy would actually have worn, so the
# honest default is to KEEP it. Set drop_suspect=True to exclude it and quantify
# how much of the drawdown one weekend accounts for -- but report the inclusive
# number as the headline.
SUSPECT_GAP_SESSIONS = {
    "NQ": ["2026-06-15"],
    "ES": ["2026-06-15"],
    "CL": ["2026-06-15"],
    "GC": ["2026-06-15"],
...
def add_features(df, tf=5, instrument="NQ"):
    ...
    d["suspect"] = d["session"].dt.strftime("%Y-%m-%d").isin(
        SUSPECT_GAP_SESSIONS.get(instrument.upper(), []))
    return d
...
def simulate(df, signals, mode="atr", skip_dead=True, dead_col="dead",
             cluster=2, cost=None, drop_suspect=False, size="micro",
    ...
    """
    if cost is None:      # resolve per instrument; never inherit NQ's point cost
        inst = df["instrument"].iloc[0] if "instrument" in df.columns else "NQ"
        cost = cost_for(inst, size)
