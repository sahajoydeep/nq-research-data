...
# ---------------------------------------------------------------- loading
def load(tf=5, path=None, drop_last=True, tz="Asia/Kolkata"):
    """Load a timeframe master (or an explicit path). Returns tz-aware IST frame."""
    f = path or os.path.join(UPLOADS, MASTERS[tf])
    df = pd.read_csv(f)
    ...
    df["tf"] = tf
    return add_features(df, tf)


def add_features(df, tf=5):
    d = df.copy()
    t = d["time"]
